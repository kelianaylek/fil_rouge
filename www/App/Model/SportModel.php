...
class SportModel extends Database{

    // Select all the polls created by the user 
    public function getAllUserPolls($userId, $currentTime){
        $reqAllPolls = $this->query("SELECT * FROM polls WHERE accepted_id = '$userId' AND poll_limit > '$currentTime' AND poll_type = 'sport' ORDER BY poll_id DESC ");
        return ($reqAllPolls);
    }
    ...
    public function currentFriendsPolls($friendId, $currentDateTime){
        $reqCurrentFriendsPolls = $this->query("SELECT * FROM polls WHERE accepted_id = '$friendId' AND poll_limit > '$currentDateTime' AND poll_type = 'sport' ORDER BY created_at DESC");
