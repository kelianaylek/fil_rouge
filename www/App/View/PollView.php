<!DOCTYPE html>
<html lang="fr">

<head>
    <meta charset="UTF-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1.0" />
    <link rel="stylesheet" href="../../public/assets/css/style.css" />
    <link href="https://fonts.googleapis.com/css2?family=Open+Sans:ital,wght@0,300;0,400;0,600;0,700;0,800;1,300;1,400;1,600;1,700;1,800&display=swap" rel="stylesheet" />
    <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/font-awesome/4.7.0/css/font-awesome.min.css" />
    <title>Projet fil rouge</title>
</head>

<body>
    <header>
        <nav>
            <a href="index.html"><i class="fa fa-chevron-left" aria-hidden="true"></i></a>
            <div>
                <a href="shopping.html"><i class="fa fa-shopping-cart fa-lg" aria-hidden="true"></i></a>
                <a href=""><p>0</p><i class="fa fa-star fa-lg" aria-hidden="true"></i></a>
                <a href=""><i class="fa fa-user-circle-o fa-lg" aria-hidden="true"></i></a>
            </div>
        </nav>
    </header>

    <main class="mainPageSondage">
        <!-- en fixed -->
        <div>
            <h1>Paris - Marseille</h1>
            <h2>Après-demain 19:00</h2>
        </div>
        <!-- en fixed -->
        <div class="colonnesPageSondage">

            <!-- <section class="container">
                <div>
                    <h3>Les deux équipes marquent</h3>
                    <div>
                        <i class="fa fa-star" aria-hidden="true"></i>
                        <i class="fa fa-star" aria-hidden="true"></i>
                        <i class="fa fa-star" aria-hidden="true"></i>
                        <i class="fa fa-star-o" aria-hidden="true"></i>
                        <i class="fa fa-star-o" aria-hidden="true"></i>
                    </div>
                </div>
                <form action="" name="formChoice">
                    <button type="submit" name="formLeftChoice">Choix 1</button>
                    <button type="submit" name="formRightChoice">Choix 2</button>
                </form>
            </section> -->
        </div>


    </main>


    <?php
        include "inc/footer.inc.php"
    ?>

    <script src="js/index.js"></script>

</body>